...
export async function initPronostici() {
  ...
  // Popola classifiche girone dai pronostici già salvati
  Object.keys(DB.gironi).forEach(lettera => _ricalcolaClassificaGirone(lettera));
...
function _bindSegniGirone() {
  ...
  document.querySelectorAll('.score-input').forEach(input => {
    input.addEventListener('input', () => {
      ...
      // Ricalcola classifica del girone in tempo reale
...
// ── CLASSIFICA GIRONE IN TEMPO REALE ─────────────────
...
function _ricalcolaClassificaGirone(lettera) {
  ...
  girone.partite.forEach(p => {
    ...
    else if (gc === gt) { stats[p.casa].pt += 1; stats[p.trasferta].pt += 1; }
    else              { stats[p.trasferta].pt += 3; }
  });

  // Ordina: punti → differenza reti → gol fatti
  const classifica = girone.squadre
    .map(id => ({ id, ...stats[id] }))
    .sort((a, b) => b.pt - a.pt || b.gd - a.gd || b.gf - a.gf);

  const hasData = classifica.some(t => t.g > 0);

  // ── Aggiorna mini-tabella nel tab Gironi ──
  ...
  if (miniEl) {
    if (!hasData) {
      ...
      miniEl.innerHTML = `
        <table class="girone-mini-table">
          <thead>
            <tr><th>#</th><th>Squadra</th><th>Pt</th><th>G</th><th>GF</th><th>GS</th><th>GD</th></tr>
          </thead>
          <tbody>
            ${classifica.map((t, i) => {
              const sq  = DB.squadre[t.id];
              const gd  = t.gd > 0 ? `+${t.gd}` : `${t.gd}`;
              const cls = i < 2 ? 'qualificata' : '';
              return `<tr class="${cls}">
                ...
                <td class="${t.gd > 0 ? 'gd-pos' : t.gd < 0 ? 'gd-neg' : ''}">${gd}</td>
              </tr>`;
            }).join('')}
          </tbody>
        </table>`;
  ...
  // ── Auto-compila le posizioni nel tab Speciali ──
